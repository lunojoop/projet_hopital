<?php
    if (isset($_POST['supprime']))
    {
        ...
        $id_specialite = htmlspecialchars(trim($_POST['id_specialite']));
        ...
        if (preg_match("/^[a-zA-Z\S ]+$/", $nom))
        {
			if (preg_match("/^[a-zA-Z\S ]+$/", $prenom))
            {
                if (!empty($login) AND !empty($mopass))
                {
                    ...
                    $req='DELETE FROM `medecin` WHERE id_medecin='.$id_medecin.'';
                    $inser=$db->getPDO()->prepare($req);
                    $inser->execute(array($id_medecin,$nom,$prenom,$email,$login,$mopass,$id_service,$id_specialite,$id_admin));
...
<head>
	<meta charset="utf-8">
	<title>Supprimer un Medecin</title>
...
<body>
	...
	<form method="post" action="">
        <table>
            <tr><td><label for="id_medecin">ID_Medecin<input type="Text" name="id_medecin"></label></br></br></br></td></tr>
            ...
            <tr><td><label for="id_specialite">Specialité
                <select name="id_specialite" id="id_specialite">
                    <option name="id_specialite[]" value="1">Cardiologue</option>
                    <option name="id_specialite[]" value="2">Gastroentérologue</option>
                    <option name="id_specialite[]" value="3">dermatologue</option>
                    <option name="id_specialite[]" value="4">Neurologue</option>
                    <option name="id_specialite[]" value="5">Allergologue</option>
                    <option name="id_specialite[]" value="6">Sexlogue</option>
                </select>
                </label></br></br></br></td><tr>
            ...
            <tr><td><input type="submit" name="supprime" value="Supprimer"></td><tr>
        </table>
	</form>
