        ...
        if (preg_match("/^[a-zA-Z\S ]+$/", $nom))
        {
			if (preg_match("/^[a-zA-Z\S ]+$/", $prenom))
            {
                if (!empty($login) AND !empty($mopass))
                {
                    ...
                    $req='INSERT INTO `medecin`(`nom`, `prenom`, `email`, `login`, `mopass`, `id_service`, `id_specialite`, `id_admin`) VALUES (?,?,?,?,?,?,?,?)';
                    $inser=$db->getPDO()->prepare($req);
                    $inser->execute(array($nom,$prenom,$email,$login,$mopass,$id_service,$id_specialite,$id_admin));
