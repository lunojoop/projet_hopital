        ...
        if (preg_match("/^[a-zA-Z\S ]+$/", $nom))
        {
			if (preg_match("/^[a-zA-Z\S ]+$/", $prenom))
            {
                if (!empty($login) AND !empty($mopass))
                {
                    ...
                    $req='DELETE FROM `secretaire` WHERE id_secretaire='.$id_secretaire.'';
...
<head>
	<meta charset="utf-8">
	<title>Supprimer une Secrétaire</title>
...
<body>
	...
	<form method="post" action="">
        <table>
            <tr><td><label for="id_secretaire">ID_Secretaire<input type="Text" name="id_secretaire"></label></br></br></br></td></tr>
